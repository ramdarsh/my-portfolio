...
(function initParticleBackground() {
    ...
    // Check if separate particles are close enough to draw line
    function connect() {
        let opacityValue = 1;
        for (let a = 0; a < particlesArray.length; a++) {
            for (let b = a; b < particlesArray.length; b++) {
                ...
                if (window.innerWidth < 768) {
                    connectDistance = (canvas.width / 4.5) * (canvas.height / 4.5);
                }
    ...
    function init() {
        ...
        // Limit particles on mobile devices for performance
        if (window.innerWidth < 768) {
        ...
        for (let i = 0; i < numberOfParticles; i++) {
            ...
            let directionX = (Math.random() * 0.8) - 0.4; // Slower speed
